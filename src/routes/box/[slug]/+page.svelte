<script>
  ...
  const splicePhoto = (index) =>{
	...
	newPhotos = newPhotos.filter(photo => {photo !== photos[index]; isNewPhoto = true;})
				...
				{#each photos as photo, index}
					<span class="imgStack">
						<span class="btnWrapper" style="padding-top:1em; padding-right:1em;">
							<Button kind="danger-tertiary" iconDescription="Delete" icon={TrashCan} on:click={()=>{splicePhoto(index)}}/>
						</span>			
